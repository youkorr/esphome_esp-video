"""A copy of portall under its own name, to experiment on.

portall is what people are running. This exists so the work on a panel as a
screen for a PC -- over Wi-Fi, driven by a program on that PC rather than by
a Home Assistant add-on -- cannot break it. Everything here started as a
byte-for-byte copy; only the names differ:

    namespace portall        ->  namespace wired_portall
    class Portall            ->  class WiredPortall
    portall.sleep / .wake    ->  wired_portall.sleep / .wake
    number: platform: portall->  number: platform: wired_portall
    PORTALL_AUDIO_*          ->  WIRED_PORTALL_AUDIO_*

What deliberately did NOT move is components/usb_display_tusb and every
CONFIG_USB_DISPLAY_* symbol: that is the TinyUSB descriptor component, both of
these depend on it, and it is shared rather than duplicated.

The two cannot be in one firmware -- see _not_beside_portall below, which says
so at validation rather than letting the linker say it.

yaml/ws-wired-portall.yaml is a validated example.
...
wired_portall_ns = cg.esphome_ns.namespace("wired_portall")
WiredPortall = wired_portall_ns.class_("WiredPortall", cg.Component)
...
SleepAction = wired_portall_ns.class_("SleepAction", automation.Action)
WakeAction = wired_portall_ns.class_("WakeAction", automation.Action)

_AWAKE_ACTION_SCHEMA = automation.maybe_simple_id(
    {cv.Required(CONF_ID): cv.use_id(WiredPortall)}
)
...
@automation.register_action(
    "wired_portall.sleep", SleepAction, _AWAKE_ACTION_SCHEMA, synchronous=True
)
@automation.register_action(
    "wired_portall.wake", WakeAction, _AWAKE_ACTION_SCHEMA, synchronous=True
)
async def wired_portall_awake_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
    return var


# wired_portall.set_volume, for a volume somebody wants to own themselves: a template
...
SetVolumeAction = wired_portall_ns.class_("SetVolumeAction", automation.Action)


@automation.register_action(
    "wired_portall.set_volume",
    SetVolumeAction,
    cv.Schema(
        {
            cv.GenerateID(): cv.use_id(WiredPortall),
...
async def wired_portall_set_volume_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
def _not_beside_portall(config):
    """Refuse a firmware carrying this AND portall, with the reason.

    They are the same component under two names, so the C++ they compile
    defines the same `extern "C"` TinyUSB callbacks -- tud_vendor_rx_cb,
    tud_mount_cb, the whole tud_msc_ and tud_hid_ family. Those are C symbols
    at global scope, so a firmware with both does not link.

    `esphome config` passes on such a configuration, because it validates YAML
    and codegen and never compiles C++ -- checked, it really does pass. So the
    failure would arrive as a duplicate-symbol error from the linker, at the
    end of a long build, naming a symbol that appears in neither YAML nor any
    file the person edited. One line here is worth that.
    """
    if "portall" in fv.full_config.get():
        raise cv.Invalid(
            "wired_portall and portall cannot be in the same firmware: they "
            "are the same component under two names and define the same USB "
            "callbacks, so the build would fail at the link with a duplicate "
            "symbol. Keep one. wired_portall is the copy to experiment on; "
            "portall is the one to run."
        )
    return config


FINAL_VALIDATE_SCHEMA = _not_beside_portall


CONFIG_SCHEMA = cv.All(
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(WiredPortall),
