...
static const char *const TAG = "wired_portall.net";
...
void WiredPortall::queue_touch_(const touchscreen::TouchPoints_t &points) {
...
void WiredPortall::set_awake(bool awake) {
...
void WiredPortall::send_queued_messages_(int client) {
...
void WiredPortall::setup_network_() {
  if (this->port_ == 0)
    return;
#ifdef USE_TOUCHSCREEN
  ...
  xTaskCreatePinnedToCore(WiredPortall::network_task, "udispnet", 4096, this, 4, nullptr, tskNO_AFFINITY);
  ESP_LOGCONFIG(TAG, "Listening on port %u for frames", (unsigned) this->port_);
}

void WiredPortall::network_task(void *param) { static_cast<WiredPortall *>(param)->run_network_task(); }

void WiredPortall::run_network_task() {
