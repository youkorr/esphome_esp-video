...
static const char *const TAG = "wired_portall.audio";
...
WiredPortall *audio_owner(void *ctx) { return static_cast<WiredPortall *>(ctx); }
...
void WiredPortall::setup_speaker_() {
  ...
  if (this->speaker_ == nullptr || this->audio_block_ != nullptr)
    return;
  this->audio_block_size_ = (size_t) (WIRED_PORTALL_AUDIO_RATE / 1000) * AUDIO_BLOCK_MS *
                            (WIRED_PORTALL_AUDIO_BITS / 8) * WIRED_PORTALL_AUDIO_CHANNELS;
  ...
  this->speaker_->set_audio_stream_info(
      audio::AudioStreamInfo(WIRED_PORTALL_AUDIO_BITS, WIRED_PORTALL_AUDIO_CHANNELS, WIRED_PORTALL_AUDIO_RATE));
  ESP_LOGCONFIG(TAG, "Speaker: %d Hz, %d bit, %d channel, %u byte blocks", WIRED_PORTALL_AUDIO_RATE, WIRED_PORTALL_AUDIO_BITS,
                WIRED_PORTALL_AUDIO_CHANNELS, (unsigned) this->audio_block_size_);
}

#if CFG_TUD_AUDIO
void WiredPortall::setup_uac_() {
...
void WiredPortall::on_audio_samples(const uint8_t *data, size_t length) {
  ...
  if (!this->logged_first_audio_) {
    ...
    ESP_LOGI(TAG, "First audio: %u byte packets at %d Hz, %d bit, %d channel, played %u at a time",
             (unsigned) this->last_packet_len_, WIRED_PORTALL_AUDIO_RATE, WIRED_PORTALL_AUDIO_BITS, WIRED_PORTALL_AUDIO_CHANNELS,
             (unsigned) this->audio_block_size_);
  }
}

void WiredPortall::flush_audio_block_() {
  ...
  if (written == 0 && carried >= this->audio_block_size_) {
    ...
    if (this->audio_resyncs_ == 1 || this->audio_resyncs_ % 100 == 0) {
      if (this->audio_ever_accepted_) {
        ...
        // that returns zero for as long as the board is up. wired_portall sends
        ...
        ESP_LOGE(TAG,
                 ...
                 "falling behind: wired_portall sends %d Hz, %d bit, %d channel, and an ESPHome mixer refuses a source "
                 ...
                 "component above.",
                 (unsigned) this->audio_resyncs_, WIRED_PORTALL_AUDIO_RATE, WIRED_PORTALL_AUDIO_BITS, WIRED_PORTALL_AUDIO_CHANNELS);
      }
    }
  }
}

void WiredPortall::set_audio_volume(float volume) {
...
void WiredPortall::on_usb_audio_mute(bool muted) {
