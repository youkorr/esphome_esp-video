...
from .. import WiredPortall, wired_portall_ns

DEPENDENCIES = ["wired_portall"]

USBVolumeNumber = wired_portall_ns.class_("USBVolumeNumber", number.Number, cg.Component)

CONF_WIRED_PORTALL_ID = "wired_portall_id"
...
CONFIG_SCHEMA = cv.All(
    ...
    .extend(
        {
            cv.GenerateID(CONF_WIRED_PORTALL_ID): cv.use_id(WiredPortall),
            cv.Optional(CONF_USB_DISPLAY_ID): cv.use_id(WiredPortall),
        }
    )
...
async def to_code(config):
    ...
    parent = await cg.get_variable(
        config.get(CONF_USB_DISPLAY_ID) or config[CONF_WIRED_PORTALL_ID]
    )
