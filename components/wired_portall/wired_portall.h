 ...
 * two megabytes. */
#define WIRED_PORTALL_AUDIO_RATE 48000
#define WIRED_PORTALL_AUDIO_BITS 16
#define WIRED_PORTALL_AUDIO_CHANNELS 1
 ...
 */
class WiredPortall : public Component
#ifdef USE_TOUCHSCREEN
    ,
...
/// of opposites, and it keeps the YAML reading as "wired_portall.sleep" without
/// an argument to get the wrong way round.
template<typename... Ts> class SleepAction final : public Action<Ts...>, public Parented<WiredPortall> {
 public:
  void play(const Ts &...) override { this->parent_->set_awake(false); }
};

template<typename... Ts> class WakeAction final : public Action<Ts...>, public Parented<WiredPortall> {
 public:
  void play(const Ts &...) override { this->parent_->set_awake(true); }
};

#ifdef USE_SPEAKER
/* wired_portall.set_volume, so a template number can own the volume.
 *
 * The component's own `number: platform: wired_portall` follows whatever the sound
 ...
 */
template<typename... Ts> class SetVolumeAction final : public Action<Ts...>, public Parented<WiredPortall> {
 public:
