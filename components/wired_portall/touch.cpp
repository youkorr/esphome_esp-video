...
static const char *const TAG = "wired_portall.touch";

void WiredPortall::setup_touch_() {
...
void WiredPortall::update(const touchscreen::TouchPoints_t &points) {
#if CFG_TUD_HID
...
#if CFG_TUD_HID
bool WiredPortall::send_touch_report_(const udisp_touch_report_t &report) {
  if (!tud_hid_ready())
    return false;
  return tud_hid_report(REPORT_ID_TOUCH, &report, sizeof(report));
}

void WiredPortall::retry_release_() {
