...
namespace esphome {
namespace wired_portall {

static const char *const TAG = "wired_portall";
...
// has to be reachable from file scope. A second wired_portall would need a second
// USB device controller, which the ESP32-P4 does not have; the Python side
// enforces a single instance.
static WiredPortall *g_wired_portall = nullptr;

extern "C" void tud_vendor_rx_cb(uint8_t itf, uint8_t const *buffer, uint16_t bufsize) {
  (void) buffer;
  (void) bufsize;
  if (g_wired_portall != nullptr)
    g_wired_portall->on_vendor_rx(itf);
}
...
extern "C" void tud_mount_cb(void) {
  ...
  if (g_wired_portall != nullptr)
    g_wired_portall->set_configured();
...
void WiredPortall::setup() {
  if (this->display_ == nullptr) {
    this->mark_failed(LOG_STR("No display"));
    return;
  }
  g_wired_portall = this;
  ...
  xTaskCreatePinnedToCore(WiredPortall::decode_task, "udisp", 4096, this, 4, nullptr, 1);
...
bool WiredPortall::allocate_rotation_() {
...
void WiredPortall::place_(const Frame &frame, uint16_t &x, uint16_t &y, uint16_t &w, uint16_t &h) const {
...
bool WiredPortall::rotate_(const Frame &frame, uint16_t padded_width, uint16_t padded_height) {
...
bool WiredPortall::allocate_frames_() {
...
WiredPortall::Frame *WiredPortall::take_empty_(uint32_t wait_ms) {
  Frame *frame = nullptr;
...
void WiredPortall::queue_filled_(Frame *frame) {
  if (xQueueSend(this->filled_queue_, &frame, 0) != pdTRUE)
    xQueueSend(this->empty_queue_, &frame, 0);
}

bool WiredPortall::append_(Frame *frame, const uint8_t *data, size_t len) {
...
void WiredPortall::feed_(const uint8_t *data, size_t len, bool may_wait) {
  if (len == 0)
    return;
...
void WiredPortall::on_vendor_rx(uint8_t itf) {
...
// ===========================================================================
void WiredPortall::decode_task(void *param) { static_cast<WiredPortall *>(param)->run_decode_task(); }

void WiredPortall::run_decode_task() {
...
void WiredPortall::reset_stream_() {
  LockGuard lock(this->feed_lock_);
  if (this->current_ != nullptr) {
...
void WiredPortall::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "WiredPortall:");
  ...
  if (this->port_ != 0) {
    ...
    ESP_LOGCONFIG(TAG, "  Sender: the WiredPortall add-on, with width: %u and height: %u%s",
                  (unsigned) this->render_width_, (unsigned) this->render_height_,
