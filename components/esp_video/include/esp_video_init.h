...
/**
 * @brief [YOUKORR/ESPHome] Reconfigure the ISP pipeline for a given sensor.
 *
 * Compatibility shim kept for the ESPHome consumer (esp_cam_sensor_camera.cpp),
 * which calls this after switching an OV5647 custom format. The youkorr ISP/IPA
 * pipeline controller it relied on is NOT yet ported onto esp_video 2.2.0, so the
 * current implementation is a no-op stub (see esp_video_youkorr_compat.c and
 * MIGRATION_2.2.0.md). To restore per-sensor ISP re-init, reintegrate the IPA
 * pipeline controller + embedded JSON configs with the 2.2.0 ISP device.
 *
 * @param sensor_name Sensor name (e.g. "OV5647")
 * @return ESP_OK
 */
