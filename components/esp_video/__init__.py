...
CONF_H264_VERSION = "h264_version"
...
CONFIG_SCHEMA = cv.All(
    cv.Schema({
        ...
        # Hardware H.264 encoder video device (/dev/video11) on the ESP32-P4.
        # Off by default so existing MIPI/JPEG-only builds are unaffected: it
        # pulls in the espressif/esp_h264 managed component.
        cv.Optional(CONF_ENABLE_H264, default=False): cv.boolean,
        cv.Optional(CONF_H264_VERSION, default="1.3.6"): cv.string,
...
async def to_code(config):
    ...
    # Hardware H.264: CMake cannot see the -D flags posed below (they are not in
    # sdkconfig), so the choice is handed to CMakeLists.txt through a marker file
    # written next to it. That keeps `esp_video_h264_device.c` and its esp_h264
    # dependency out of the build entirely when H.264 is not wanted.
    _h264_marker = os.path.join(component_dir, ".enable_h264")
    if config[CONF_ENABLE_H264]:
        with open(_h264_marker, "w", encoding="utf-8") as marker:
            marker.write("Generated by the ESPHome esp_video component; see enable_h264.\n")
    elif os.path.exists(_h264_marker):
        os.remove(_h264_marker)
    ...
    # Espressif's H.264 encoder library, which backs /dev/video11 on the P4.
    if config[CONF_ENABLE_H264]:
        esp32.add_idf_component(name="espressif/esp_h264", ref=config[CONF_H264_VERSION])
    ...
    # Encodeur H.264 matériel (ESP32-P4) -> /dev/video11
    if config[CONF_ENABLE_H264]:
        flags.extend([
